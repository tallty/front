...
const useListLayout = (props: VObject, emit: any) => {
  ...
  const key = computed(() =>
    [props.item.model_key_prefix, props.item.model_key]
      .filter((i) => i)
      .join('.')
  );
  ...
  const disabled = computed(
    () => (inject('isViewer') as boolean) || accessibilityDisabled.value
  );
  ...
  const records: WritableComputedRef<VObject[]> = computed({
    get: () => {
      ...
      for (let i = 0; i < (getKey(key.value) || []).length; i++) {
        cardTemplateAry.value[i] =
          cardTemplateAry.value[i] || cloneDeep(props.item);
      }
  ...
  const update = async (payload: VObject) => {
    const index = records.value.findIndex(
      (record) =>
        (record._id && record._id === payload._id) ||
        (record.id && record.id === payload.id)
    );
    emit('update', { ...payload });
  ...
  const resetFormData = (value: VObject = {}) => {
    ...
    Object.keys(value).forEach(
      (key) => (fakeRecord.value.formData[key] = value[key])
    );
  };
