...
const ComComRouteSettingsIndex = defineComponent({
  name: 'ComComRouteSettingsIndex',
  components: {
  ...
  setup(props, { emit }) {
    ...
    const dataSource = computed(() =>
      props.store.record.value.conf?.actions.map((item: VObject) => ({
        ...
        mname: props.store.record.value.mod_name,
        ...item,
      })),
    );
...
export default ComComRouteSettingsIndex;
</script>
