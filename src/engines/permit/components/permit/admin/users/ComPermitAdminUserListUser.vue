...
const ComPermitAdminUserListUser = defineComponent({
  name: 'ComPermitAdminUserListUser',
  components: {},
  props: {
  ...
  setup(props, { emit }) {
    ...
    const config = computed(() => ({
      ...
      params: {
        q: {
          roles_id_in: [props.activeRole?.id],
        },
      },
      table: {
        pushColumns: [
    ...
    const delItem = (v: any) => {
      emit('delRoleRelation', 'user', props.activeRole?.id, v?.id);
    };
    ...
    const onSearch = () => {
      (taIndexView.value as any).fetchData(1, { name_or_account_or_mobile_cont: searchKey.value });
    };
...
export default ComPermitAdminUserListUser;
</script>
