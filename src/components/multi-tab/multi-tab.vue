        ...
        <a-dropdown>
          ...
          <template #overlay>
            <a-menu>
              ...
              <a-menu-item
                key="close-other"
                @click="closeOther(route.fullPath)"
                :disabled="cacheListLength === 1"
              >
                关闭其他
              </a-menu-item>
        ...
        <template #tab>
          <a-dropdown :trigger="['contextmenu']">
            <span class="ant-pro-multi-tab-title">
              {{ item.route.meta.title }}&nbsp
              <reload-outlined
                key="reload"
                v-if="
                  store.current.replace(/\?.+$/, '') === item.route.fullPath.replace(/\?.+$/, '')
                "
                class="ant-pro-multi-tab-reload-btn"
                @click="handleReloadPage(item.route.fullPath)"
                :spin="spin"
              />
              <close-outlined
                key="close"
                ...
                @click.stop="e => handleClose(e, item.route.fullPath)"
              />
            </span>
            <template #overlay>
              <a-menu>
                ...
                <a-menu-item
                  key="close-left"
                  :disabled="index === 0"
                  @click="closeLeft(item.route.fullPath)"
                >
                  关闭到左侧
                </a-menu-item>
                <a-menu-item
